...
function [total_hit, ToT_center, degree1_ToAdist, degree2_ToAdist,...
    degree3_ToAdist,early_pixels] = decluster2(ToT,ToA,neighbouring_degree,...
...
%      pixels_that_time = find((ToA == ToA_1D_U(time)));% & (complete_mark == 0)));
% end
% toc
% toc
% toc
...
%     pixels_that_time = find((ToA == ToA_1D_U(time)));% & (complete_mark == 0)));
    pixels_that_time = ToA_1D_I(ia(left(time):right(time)));
%     [C,R] = ind2sub(size(ToA), pixels_that_time);
    for i = 1:1:length(pixels_that_time)
        c = floor(pixels_that_time(i)/row_size) + 1;
        r = mod(pixels_that_time(i),row_size);
        if (r == 0)
            r = row_size;
        end
