...
function [total_hit, ToT_center, degree1_ToAdist, degree2_ToAdist,...
    degree3_ToAdist,early_pixels] = decluster(ToT,ToA,neighbouring_degree,...
...
for time = 1:1:length(ToA_1D)
    pixels_that_time = (ToA == ToA_1D(time));
    
    for r = 1:1:row_size
        for c = 1:1:column_size
            if((pixels_that_time(r,c) && (~complete_mark(r,c))))
